...
class ActividadController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            // centro_estudiantes solo puede publicar en su propia carrera
            if ($rolActivo === 'centro_estudiantes') {
                $data['carrera_id'] = $usuarioActual->carrera_id;
            }

            // centro_facultativo solo puede publicar en carreras de su facultad
            if ($rolActivo === 'centro_facultativo') {
                $data['carrera_id'] = null; // aplica a toda la facultad
                // Si envía un carrera_id, verificar que pertenezca a su facultad
                if (!empty($data['carrera_id'])) {
                    $carrera = \App\Models\Carrera::find($data['carrera_id']);
                    if (!$carrera || (int) $carrera->facultad_id !== (int) $usuarioActual->facultad_id) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Solo puedes publicar en carreras de tu facultad.',
                        ], 403);
                    }
                }
            }
