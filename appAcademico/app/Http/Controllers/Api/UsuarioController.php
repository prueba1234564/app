...
class UsuarioController extends Controller
{
    ...
    /**
     * POST /usuarios/{usuario}/asignar-centro-estudiantes
     * Permite a centro_estudiantes o centro_facultativo asignar/quitar
     * el rol centro_estudiantes a un estudiante de su carrera.
     */
    public function asignarCentroEstudiantes(Request $request, int $id): JsonResponse
    {
        try {
            $usuarioActual = $request->user();
            $habilidades   = $usuarioActual?->currentAccessToken()?->abilities ?? [];
            ...
            // Solo centro_estudiantes y centro_facultativo pueden usar este endpoint
            if (!in_array($rolActivo, ['centro_estudiantes', 'centro_facultativo'])) {
                return response()->json([
                    'success' => false,
            ...
            $usuario = Usuario::with('rolesUsuario')->find($id);
            if (!$usuario) {
                return response()->json([
                    'success' => false,
                    'message' => 'Usuario no encontrado.',
            ...
            // Verificar que el estudiante pertenece a la misma carrera
            if ($rolActivo === 'centro_estudiantes') {
                if ((int) $usuario->carrera_id !== (int) $usuarioActual->carrera_id) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Solo puedes asignar roles a estudiantes de tu carrera.',
                    ], 403);
                }
            }

            // centro_facultativo verifica que el estudiante sea de su facultad
            if ($rolActivo === 'centro_facultativo') {
                if ((int) $usuario->facultad_id !== (int) $usuarioActual->facultad_id) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Solo puedes asignar roles a estudiantes de tu facultad.',
                    ], 403);
                }
            }

            // Verificar que el usuario objetivo tiene rol estudiante
            $esEstudiante = $usuario->rolesUsuario->contains('rol', 'estudiante');
            if (!$esEstudiante) {
                return response()->json([
                    'success' => false,
                    'message' => 'Solo puedes asignar este rol a estudiantes.',
                ], 422);
            }
            ...
            if ($accion === 'asignar') {
                // Evitar duplicado
                $yaExiste = $usuario->rolesUsuario->contains('rol', 'centro_estudiantes');
                if (!$yaExiste) {
                    RolUsuario::create([
                        'usuario_id'  => $usuario->id,
                        'rol'         => 'centro_estudiantes',
                        'carrera_id'  => $usuario->carrera_id,
                        'facultad_id' => $usuario->facultad_id,
                    ]);
                }
                $mensaje = 'Rol centro_estudiantes asignado correctamente.';
            } else {
                // Quitar el rol
                $usuario->rolesUsuario()
                    ->where('rol', 'centro_estudiantes')
                    ->delete();
                $mensaje = 'Rol centro_estudiantes removido correctamente.';
            }
