...
const DESTINOS = [
  ...
  { value: 'todos', label: 'Todos en la carrera' },
];
...
export default function NotificacionCentroScreen({ navigation }) {
  ...
  const confirmSend = async () => {
    ...
    try {
      ...
      setTitulo('');
      setCuerpo('');
      Alert.alert('✓ Enviada', `Notificación enviada a ${destino === 'estudiantes' ? 'los estudiantes' : 'todos en la carrera'}.`, [
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          ...
          <View style={s.radioGroup}>
            {DESTINOS.map((item) => (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text style={s.destinatarioTitle}>{item.label}</Text>
                  <Text style={s.destinatarioSub}>
                    {item.value === 'estudiantes'
                      ? `Solo estudiantes de ${carreraNombre}`
                      : `Todas las personas vinculadas a ${carreraNombre}`}
                  </Text>
                </View>
              </Pressable>
            ))}
          ...
          <Text style={s.infoText}>
            {destino === 'estudiantes'
              ? 'Se enviará solo a los estudiantes registrados en la carrera.'
              : 'Se enviará a todos los usuarios asignados a la carrera.'}
          </Text>
        </View>
...
const s = StyleSheet.create({
  ...
  infoText: { fontSize: 13, color: '#475569', marginTop: 12 },
  lockedBadge: {
