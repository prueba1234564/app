...
export default function MateriaPeriodoScreen({ navigation }) {
  ...
  const cargar = useCallback(async (periodoIdOverride = null) => {
    setLoading(true);
    try {
      // Primero cargar materias para obtener el tipo de carrera
      ...
      // Obtener tipo de carrera desde la primera materia o desde el usuario
      const tipoCarrera = materiasData[0]?.carrera?.tipo ?? null;
      const tipoParam = tipoCarrera ? `?tipo_carrera=${tipoCarrera}` : '';

      const [periodoRes, todosRes] = await Promise.all([
        api.get(`/periodos/activo${tipoParam}`).catch(() => ({ data: { data: null } })),
      ...
      {periodo && (
        <View style={[styles.periodoCard, !periodo.activo && styles.periodoCardHistorico]}>
          ...
          <Text style={[styles.periodoText, !periodo.activo && { color: '#94a3b8' }]}>
            {periodo.tipo === 'semestre' ? 'Semestral' : periodo.tipo === 'anual' ? 'Anual' : 'Temporada'} — {periodo.fecha_inicio} al {periodo.fecha_fin}
          </Text>
          {!periodo.activo && (
